...
本指引涵蓋老虎機負責任博弈功能的完整實作工作流程，包括存款限制、自我排除機制、會話時間限制、勝負追蹤、自動播放管控與風險訊息顯示。所有功能均以保護玩家為核心目標，並符合主要司法管轄區的監管要求。
...
自我排除（Self-Exclusion）允許玩家主動暫停或永久停止遊戲存取。此機制是負責任博弈的核心功能，為所有主要司法管轄區的強制要求。
...
風險訊息顯示（Risk Message Display）在遊戲介面中向玩家展示 RTP 百分比與博弈風險提示，確保玩家在充分知情的情況下進行遊戲。
...
|----------|----------|
| 一般風險提示 | 「博弈具有風險，請量力而為」 |
| 虧損提醒 | 「長期而言，博弈的預期結果為虧損」 |
| 成癮警告 | 「博弈可能導致成癮，如需協助請撥打求助熱線」 |
| 年齡限制 | 「本遊戲僅限 18 歲（或 21 歲）以上人士」 |
| 求助資訊 | 「如果您覺得博弈已影響您的生活，請聯繫 [求助機構名稱]」 |
...
實作負責任博弈功能後，請使用以下檢查清單確認所有必要功能均已完成：
